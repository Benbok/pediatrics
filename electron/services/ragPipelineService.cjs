...
- Если в [КОНТЕКСТ] есть хотя бы частичная релевантная информация — ОБЯЗАТЕЛЬНО выдай её с цитатами [N].
- Фразу «Данные отсутствуют в предоставленных материалах.» используй ТОЛЬКО если в [КОНТЕКСТ] вообще нет релевантных фрагментов.
...
- Если часть параметров (например, курс или возраст) не указана в контексте, явно напиши «не указано в предоставленных материалах», но не заменяй этим весь ответ.
...
const NO_DATA_ANSWER = 'Данные отсутствуют в предоставленных материалах.';

function isNoDataAnswer(text) {
    return String(text || '').trim() === NO_DATA_ANSWER;
}

function buildExtractiveRetryMessages(context, query) {
    return [
        {
            role: 'system',
            content:
                'Ты — клинический ассистент. Работай строго extractive-режиме: только факты из [КОНТЕКСТ], без домыслов. ' +
                `Отвечай по-русски. Если совсем нет релевантных данных, верни ровно: «${NO_DATA_ANSWER}».`,
        },
        {
            role: 'user',
            content:
                ...
                context +
                '\n\n[ЗАДАЧА]\n' +
                'Найди любые релевантные сведения по вопросу и перечисли их списком с ссылками [N]. ' +
                'Если найдено частично — выдай частичный ответ и отдельно укажи, чего не хватает в материалах.\n\n' +
                '[ВОПРОС]\n' +
                query,
        },
    ];
 ...
 */
async function rewriteQueryForRetrieval(query, queryType, llm = localLlmService) {
 ...
 */
async function expandQueryViaLlm(query, initialChunkCount = 0, llm = localLlmService) {
    const q = String(query || '').trim();
    if (q.length < 5) return null;
 ...
 */
async function retrieveAndRank({ query, diseaseId, queryVec, queryType, initialChunks, contextBudget, expandedQuery, llm = localLlmService }) {
    ...
    if (queryType !== 'list' && top.length > 0) {
        ...
        if (avgScore < CFG.selfRefineScoreThreshold) {
            ...
            const rewrittenQuery = await rewriteQueryForRetrieval(query, queryType, llm);
...
async function ragQuery({ query, diseaseId, history = [] }) {
    ...
    const { contextBudget, maxTokens: rawMaxTokens } = await resolveTokenBudgets(desiredMaxTokens, 0);

    // Resolve provider once per request
    const llm = await llmRouter.getProvider('rag');

    // Cloud models (Gemini) don't have the local token budget constraints — scale up
    const maxTokens = llm.providerType === 'gemini' ? Math.min(rawMaxTokens * 3, 4096) : rawMaxTokens;
    ...
    const expandedQuery = await expandQueryViaLlm(query, initialChunks.length, llm);

    const { top, context } = await retrieveAndRank({ query, diseaseId, queryVec, queryType, initialChunks, contextBudget, expandedQuery, llm });

    if (top.length === 0) {
        return { answer: NO_DATA_ANSWER, sources: [], context: '' };
    }
    ...
    const genOptions = { maxTokens, temperature: CFG.temperature, topP: 0.9 };
    // Some Gemini models require thinking mode and reject thinkingBudget=0.
    // Use a small non-zero budget to stay compatible across model variants.
    if (llm.providerType === 'gemini') genOptions.thinkingBudget = 128;
    ...
    if (result.status === 'error' && /400|context|too many|overflow|exceeded/i.test(result.error || '')) {
        ...
        result = await llm.generate(
            ...
            { ...genOptions, maxTokens: Math.min(maxTokens, 400) },
            (token) => { answerText += token; }
        );
    }
    ...
    // Retry once in strict extractive mode to reduce false no-data responses
    // when retrieval has relevant chunks but model is overly conservative.
    if (top.length > 0 && isNoDataAnswer(answerText)) {
        logger.warn('[RAGPipeline] No-data answer with non-empty context, retrying extractive mode', {
            diseaseId,
            queryType,
            chunks: top.length,
        });
        answerText = '';
        const retryMessages = buildExtractiveRetryMessages(context, query);
        const retryResult = await llm.generate(
            retryMessages,
            { ...genOptions, maxTokens: Math.max(600, Math.min(maxTokens, 1400)), temperature: 0.1 },
            (token) => { answerText += token; }
        );
        if (retryResult.status === 'error') throw new Error(retryResult.error || 'RAG extractive retry failed');
        if (retryResult.status === 'aborted') throw new Error('Generation aborted');
    }

    const answer = answerText.trim() || NO_DATA_ANSWER;
...
async function ragQueryStream({ query, diseaseId, history = [], onToken }) {
    ...
    if (top.length === 0) {
        if (typeof onToken === 'function') onToken(NO_DATA_ANSWER);
    ...
    // Retry once in extractive mode before emitting tokens to UI.
    if (top.length > 0 && isNoDataAnswer(streamedAnswer)) {
        logger.warn('[RAGPipeline] Stream no-data answer with non-empty context, retrying extractive mode', {
            diseaseId,
            queryType,
            chunks: top.length,
        });
        ...
        if (retryResult.status === 'error') throw new Error(retryResult.error || 'RAG stream extractive retry failed');
        if (retryResult.status === 'aborted') throw new Error('Generation aborted');
    }

    if (typeof onToken === 'function') {
        onToken(streamedAnswer.trim() || NO_DATA_ANSWER);
    }
