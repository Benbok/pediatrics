...
const CFG = {
    topK: 20,            // чанков из FTS
    topKAfterRank: 12,   // чанков в контекст после ранжирования
    temperature: 0.3,    // 0.05 слишком greedy для малых моделей
...
async function embedTextViaLmStudio(text) {
    ...
    try {
        ...
        const timeout = setTimeout(() => controller.abort(), 3000); // было 8s — сокращаем фоллбэк
        ...
        if (Array.isArray(embedding) && embedding.length > 0) {
            if (_embedCache.size >= EMBED_CACHE_MAX) {
                _embedCache.delete(_embedCache.keys().next().value); // вытесняем самую старую
            }
...
/** Получить loaded_context_length загруженной модели (cache 30s). */
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (Number.isFinite(loadedCtx) && loadedCtx > 0) {
                _modelCtxCache = { value: loadedCtx, expiresAt: now + 30_000 };
                return loadedCtx;
            }
        }
    } catch { /* fallback */ }
 ...
 * Используется как дополнительный FTS-проход для повышения recall.
 *
 * Timeout 4s, maxTokens 40 — минимальная нагрузка.
 * При ошибке или timeout возвращает null (не блокирует пайплайн).
 */
async function expandQueryViaLlm(query) {
...
async function retrieveAndRank({ query, diseaseId, queryVec, queryType, initialChunks, contextBudget, expandedQuery }) {
    const effectiveTopK = queryType === 'list' ? CFG.topK * 2 : CFG.topK;
    const effectiveTopKAfterRank = queryType === 'list' ? CFG.topKAfterRank * 2 : CFG.topKAfterRank;
...
async function ragQuery({ query, diseaseId, history = [] }) {
    ...
    const expandedQuery = await expandQueryViaLlm(query);
