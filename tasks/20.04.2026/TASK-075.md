...
### 20.04.2026 14:10 — Bugfix: повторная миграция env после удаления ключей
- Исправлен `electron/services/apiKeyStore.cjs`: `migrateFromEnv()` теперь выполняется только до первого создания store-файла
- Добавлена инициализация пустого store-файла при отсутствии env-ключей, чтобы исключить повторные автопопытки миграции
- Результат: после удаления всех ключей через UI список остаётся пустым и не пополняется из `.env`
