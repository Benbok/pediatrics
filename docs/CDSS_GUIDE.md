...
- `routeOfAdmin` - путь введения (oral, rectal, iv, im)
...
Система поддерживает три типа дозирования:

1. **Фиксированная доза** (`fixed`) - по возрасту
   - Пример: "6-12 лет: 250-500 мг"

2. **По весу** (`weight_based`) - мг/кг
   - Пример: "10 мг/кг разовая доза"

3. **По площади тела** (`bsa_based`) - мг/м²
