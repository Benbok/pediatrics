...
### Тест CDSS из терминала

Для проверки поиска по симптомам и работы ИИ без запуска приложения:

```bash
npm run test:cdss-cli
```

Введите жалобы в свободной форме (или передайте аргументом: `npx electron scripts/cdss-test-cli.cjs "температура, кашель"`). Скрипт выполнит парсинг жалоб → semantic search по базе заболеваний → ранжирование и выведет найденные карточки. Требуется `.env.local` с `VITE_GEMINI_API_KEY` и предварительно сгенерированные embeddings для заболеваний (`node scripts/generate_embeddings.cjs` при необходимости).
