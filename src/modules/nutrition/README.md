...
- Стандартные возрастные шаблоны, поставляемые модулем, теперь синхронизируются с лекцией при seed и обновляют свои пункты по title
...
Используется формула Зайцевой:

- 0.02 x масса_при_рождении_г x n

Где n - день жизни.

Ориентировочный разовый объём: 10 x n мл.
...
- Объемный метод:
  ...
  - 6 - 12 месяцев: 1/8 - 1/9 массы тела, обычно не более 1000-1100 мл/сут
- Энергетический метод:
  - 0 - 3 месяца: 120 ккал/кг/сут
  - 3 - 6 месяцев: 115 ккал/кг/сут
  - 6 - 9 месяцев: 110 ккал/кг/сут
  - 9 - 12 месяцев: 105 ккал/кг/сут

### 12-36 месяцев

- 1 - 1.5 года: 1000 - 1200 г/сут
- 1.5 - 3 года: 1200 - 1500 г/сут
- Режим: 3 основных приема пищи + 2 перекуса
- Разовая порция: до 300 - 350 мл
...
Клинические подсказки в UI:

- Для СВ и ИВ в карточке расчета отображается блок `Показания и тактика докорма`
- Туда вынесены показания к докорму, старт с 10 мл после прикладывания и приоритет сохранения лактации
...
- Seed-категории и тексты панелей синхронизированы с лекцией; пользовательские шаблоны питания врач может дополнительно корректировать вручную во вкладке `Шаблоны рациона`
- При повторном seed стандартные шаблоны прикорма и рационов 1–3 лет обновляются в соответствии с лекцией без необходимости ручного JSON-импорта
