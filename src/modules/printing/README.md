...
### 27.04.2026 — PDF visit form objective exam update
- В `visit-form` добавлен раздел «Объективный осмотр по системам».
- Показатели жизнедеятельности (ЧСС, ЧД, АД, T, SpO2) выводятся первой строкой раздела без дополнительного префикса.
- Ниже выводятся значения осмотра по системам без названий секций, каждое значение с новой строки.
- Содержимое секции осмотра по системам выводится обычным (не жирным) шрифтом.
- Раздел «Выставленный диагноз» перенесён сразу после блока «Данные пациента».
- Ослаблены ограничения на разрывы страниц для секций, чтобы убрать большие пустые области между страницами.
