...
OUT_DIR = r"C:\Users\Arty\Desktop\Django\pediatrics\src\modules\medications\data\r05cb-wave1"

# R05CB wave-1 shortlist (high-impact mucolytics/expectorants)
R05CB_WAVE1_ATC_CODES = ("R05CB01", "R05CB02", "R05CB03", "R05CB06")
...
def is_target_brand(value):
    ...
    if normalized in EXACT_BRANDS:
        return True
    for prefix in PREFIX_BRANDS:
...
def parse_pediatric_dosing(child_insuf, dosage_text, forms, route):
    ...
    for chunk in chunks:
        ...
        # Keep descriptive pediatric rules when source has child-specific guidance
        # even if explicit numeric dose is absent in text.
        descriptive_ped_ok = has_ped_signal and age_ok
        if not (dose_ok or descriptive_ped_ok):
            continue
        if not age_ok:
            continue
    ...
    if not rules and child_text and "дет" in child_text.lower():
        rules.append({
            "minAgeMonths": None,
            "maxAgeMonths": None,
            "minWeightKg": None,
...
def main():
    ...
    atc_placeholders = placeholders(R05CB_WAVE1_ATC_CODES)
