...
# All R05CB06 mucolytic/expectorant medications
R05CB06_NAMES = (
    "АМБРОБЕНЕ",
    "АМБРОБЕНЕ ФОРТЕ",
    "АМБРОКСОЛ",
    "АМБРОКСОЛ-ХЕМОФАРМ",
    "БРОНХОРУС<SUP>&reg;</SUP>",
    "ЛАЗОЛВАН<SUP>&reg;</SUP>",
    "ЛАЗОЛВАН<SUP>&reg;</SUP> МАКС",
    "ОРВИС<SUP>&reg;</SUP> БРОНХО АМБРОКСОЛ",
    "ФЛАВАМЕД<SUP>&reg;</SUP>",
)
R05CB06_ATC_CODES = ("R05CB06",)
...
def infer_age_range_months(text):
    t = (text or "").lower()
    patterns = [
        (r"от\s*(\d+)\s*месяца?\s*до\s*(\d+)\s*лет", lambda m: (int(m.group(1)), int(m.group(2)) * 12)),
        (r"от\s*(\d+)\s*[--]\s*(\d+)\s*лет", lambda m: (int(m.group(1)) * 12, int(m.group(2)) * 12)),
        (r"от\s*(\d+)\s*года?\s*до\s*(\d+)\s*лет", lambda m: (int(m.group(1)) * 12, int(m.group(2)) * 12)),
        (r"(\d+)[-–](\d+)\s*месяцев?", lambda m: (int(m.group(1)), int(m.group(2)))),
        (r"от\s*(\d+)\s*месяца?", lambda m: (int(m.group(1)), None)),
        (r"до\s*(\d+)\s*месяца?", lambda m: (0, int(m.group(1)))),
        (r"до\s*(\d+)\s*лет?", lambda m: (0, int(m.group(1)) * 12)),
        (r"старше\s*(\d+)\s*лет?", lambda m: (int(m.group(1)) * 12, None)),
    ]
...
def parse_pediatric_dosing(child_insuf, dosage_text, forms, route):
    """Enhanced pediatric dosing parser supporting weight-based and fixed-dose regimens"""
    sources = []
    ...
    for chunk in chunks:
        ...
        # Parse weight-based dosing (mg/kg)
        ...
        # Parse fixed-dose regimens (absolute mg values)
        fixed_mg_single = fixed_mg_daily = None
        m_fixed_range = re.search(r"(\d+[\.,]?\d*)\s*[--]\s*(\d+[\.,]?\d*)\s*мг\b", chunk, re.IGNORECASE)
        if m_fixed_range:
            fixed_mg_single = parse_float(m_fixed_range.group(2))  # Use upper bound as single dose
        m_daily = re.search(r"(\d+[\.,]?\d*)\s*мг\s*/\s*сут", chunk, re.IGNORECASE)
        if m_daily:
            fixed_mg_daily = parse_float(m_daily.group(1))
        ...
        has_fixed = fixed_mg_single is not None or fixed_mg_daily is not None
        if not has_weight_based and not has_fixed and not has_ped_signal:
            continue

        interval_hours = int_min
        times_per_day = infer_times_per_day(interval_hours)
        
        # Create rule with appropriate dosing type
        rule = {
            ...
            "dosing": {
                ...
                "mgPerKg": mgpk_min,
                "maxMgPerKg": mgpk_max,
            } if has_weight_based else ({"type": "fixed"} if has_fixed else None),
            "routeOfAdmin": route_val,
            "timesPerDay": times_per_day,
            "intervalHours": interval_hours,
            "maxSingleDose": fixed_mg_single,
            "maxSingleDosePerKg": mgpk_max,
            "maxDailyDose": fixed_mg_daily,
            "maxDailyDosePerKg": (mgpk_min * times_per_day) if (mgpk_min is not None and times_per_day is not None) else None,
            "instruction": chunk,
        }

        # Validation: must have dosing info and age range
        dose_ok = rule.get("dosing") is not None and rule["dosing"].get("mgPerKg") is not None if rule["dosing"] and rule["dosing"].get("type") == "weight_based" else (fixed_mg_single is not None or fixed_mg_daily is not None)
...
def main():
    ...
    log_file = os.path.join(OUT_DIR, "_generation.log")
    def log_msg(msg):
        with open(log_file, "a", encoding="utf-8") as f:
            f.write(msg + "\n")
        print(msg)
    
    log_msg(f"Starting generation at {OUT_DIR}")
    db = sqlite3.connect(f"file:{VIDAL_DB}?mode=ro", uri=True)
    db.row_factory = sqlite3.Row
    cur = db.cursor()

    name_placeholders = placeholders(R05CB06_NAMES)
    atc_placeholders = placeholders(R05CB06_ATC_CODES)
    ...
    groups = cur.execute(
        ...
        """,
        (*R05CB06_NAMES, *R05CB06_ATC_CODES),
    ).fetchall()
    ...
    for group in groups:
        ...
        active_substance = choose_active_substance(molecule_names, to_title_case(rus_name))
        ...
        file_name = f"{slug(rus_name)}_{slug(atc_code)}.json"
        out_path = os.path.join(OUT_DIR, file_name)
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        created += 1
        log_msg(f"Generated: {file_name}")

    db.close()
    log_msg(f"✓ Generated {created} files in: {OUT_DIR}")
