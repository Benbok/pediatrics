...
OUT_DIR = r"C:\Users\Arty\Desktop\Django\pediatrics\src\modules\medications\data\ambroxol"

AMBROXOL_NAMES = ("АМБРОКСОЛ",)
AMBROXOL_ATC_CODES = ("R05CB06",)
...
def infer_age_range_months(text):
    ...
    # Patterns: "от 1 месяца до 3 лет", "от 4 лет до 11 лет"
...
def parse_pediatric_dosing(child_insuf, dosage_text, forms, route):
    """Enhanced parser focusing on pediatric dosing (mg/kg and fixed doses)"""
    sources = []
    ...
    for chunk in chunks:
        ...
        # Extract weight-based dosing (mg/kg)
        ...
        # Extract fixed doses (mg)
        fixed_mg_single = None
        fixed_mg_daily = None
        m_fixed_range = re.search(r"(\d+[\.,]?\d*)\s*[-–]\s*(\d+[\.,]?\d*)\s*мг\b", chunk, re.IGNORECASE)
        if m_fixed_range:
            fixed_mg_single = parse_float(m_fixed_range.group(1))
        else:
            m_fixed_single = re.search(r"\b(\d+[\.,]?\d*)\s*мг(?:\s*(?:на|за|в|один)\s*(?:прием|раз|день))?", chunk, re.IGNORECASE)
            if m_fixed_single:
                fixed_mg_single = parse_float(m_fixed_single.group(1))
        ...
        has_ped_signal = (
            ...
            "недоношен" in chunk.lower() or
            "младенц" in chunk.lower()
        )
        ...
        dose_ok = has_weight_based or has_fixed
...
def main():
    ...
    name_placeholders = placeholders(AMBROXOL_NAMES)
    atc_placeholders = placeholders(AMBROXOL_ATC_CODES)
    ...
    groups = cur.execute(
        ...
        """,
        (*AMBROXOL_NAMES, *AMBROXOL_ATC_CODES),
    ).fetchall()
    ...
    for group in groups:
        ...
        print(f"  -> {file_name} ({len(forms)} forms, route={route_of_admin}, ped_rules={len(data['pediatricDosing'])})")
        created += 1
