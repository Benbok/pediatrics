...
OUT_DIR = r"C:\Users\Arty\Desktop\Django\pediatrics\src\modules\medications\data\paracetamol"

# Pure paracetamol (ATC N02BE01) — generic + key brands used in pediatrics
PARACETAMOL_NAMES = (
    "ПАРАЦЕТАМОЛ",
    "ПАНАДОЛ",
    "ПАРАЦЕТАМОЛ ЭВАЛАР",
    "ЦЕФЕКОН<SUP>&reg;</SUP> Д",
    "ЭФФЕРАЛГАН<SUP>&reg;</SUP>",
)
PARACETAMOL_ATC_CODES = ("N02BE01",)
...
def main():
    ...
    name_placeholders = placeholders(PARACETAMOL_NAMES)
    atc_placeholders = placeholders(PARACETAMOL_ATC_CODES)
    ...
    groups = cur.execute(
        ...
        """,
        (*PARACETAMOL_NAMES, *PARACETAMOL_ATC_CODES),
    ).fetchall()
    ...
    for group in groups:
        ...
        # Use strip_html on name for slug to handle HTML entities (e.g. &reg;)
