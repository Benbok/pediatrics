...
OUT_DIR = r"C:\Users\Arty\Desktop\Django\pediatrics\src\modules\medications\data\butamirate"

# Butamirate citrate antitussives (exclude combinations by ATC)
BUTAMIRATE_ATC_CODES = ("R05DB13",)
BUTAMIRATE_MOLECULE_LIKE = "%бутамират%"
...
def main():
    ...
    atc_placeholders = placeholders(BUTAMIRATE_ATC_CODES)
    ...
    groups = cur.execute(
        ...
        JOIN Product_ATC patc ON patc.ProductID = pd.ProductID
                ...
                WHERE patc.ATCCode IN ({atc_placeholders})
                    AND lower(mn.RusName) LIKE ?
        ORDER BY d.RusName, patc.ATCCode
        """,
                (*BUTAMIRATE_ATC_CODES, BUTAMIRATE_MOLECULE_LIKE),
    ).fetchall()
    ...
    for group in groups:
        ...
        doc_ids = [
            row[0] for row in cur.execute(
                ...
                """,
                                (rus_name, atc_code, BUTAMIRATE_MOLECULE_LIKE),
            ).fetchall()
        ]
        if not doc_ids:
            continue
