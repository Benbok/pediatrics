...
OUT_DIR = r"C:\Users\Arty\Desktop\Django\pediatrics\src\modules\medications\data\macrolides"
...
def infer_route(text):
    """
    Infer RouteOfAdmin enum value from free Russian text (ZipInfo or Dosage).
    Priority order: specific before general.
    Maps directly to valid RouteOfAdmin values:
      oral, rectal, iv_bolus, iv_infusion, iv_slow, im, sc,
      sublingual, topical, inhalation, intranasal, transdermal
    """
    t = (text or "").lower()
    ...
    # Infusion-specific text wins over bolus/slow when both are present
...
def parse_pediatric_dosing(child_insuf, dosage_text, forms, route):
    """
    Интеллектуальный парсер pediatricDosing:
    - первичный источник: ChildInsuf
    - fallback: Dosage
    - извлекает age range, мг/кг, интервалы, кратность
    """
    sources = []
    ...
    # Разбиваем на фрагменты, где обычно описываются отдельные возрастные группы
    ...
    for chunk in chunks:
        ...
        int_min, int_max = infer_interval_hours(chunk)

        # Ищем мг/кг (в т.ч. диапазон)
        ...
        # Иногда есть "начальная доза 10 мг/кг, затем по 7.5 мг/кг"
        ...
        # Базовый rule создаем, если нашли mg/kg или хотя бы детский контекст
        has_ped_signal = (
...
def main():
    ...
    docs = cur.execute(
        ...
        JOIN Product_Document pd ON pd.ProductID = pcg.ProductID
        JOIN Document d ON d.DocumentID = pd.DocumentID
        WHERE cg.ClPhGroupsID IN (187, 188)  -- макролиды + азалиды
    ...
    for doc in docs:
        ...
        # Collect ALL ZipInfo strings for route inference (some products may
        # list only в/м while others show в/в — we need the full picture)
        ...
        # Combine all ZipInfo strings for a more accurate route inference
        ...
        data = {
            ...
            "forms": forms,
            "pediatricDosing": parse_pediatric_dosing(
                d["ChildInsuf"],
                d["Dosage"],
                forms,
                inferred_route
