...
OUT_DIR = r"C:\Users\Arty\Desktop\Django\pediatrics\src\modules\medications\data\viferon"

# Viferon brand
VIFERON_NAMES = (
    "ВИФЕРОН<SUP>&reg;</SUP>",
)
VIFERON_ATC_CODES = ("L03AB05",)
...
def parse_pediatric_dosing(child_insuf, dosage_text, forms, route):
    ...
    supp_form = next((f for f in forms if isinstance(f, dict) and f.get("type") == "suppository"), None)
    form_id = (supp_form or (forms[0] if forms else {})).get("id")
    route_val = route or ("rectal" if supp_form else "oral")
    ...
    for chunk in chunks:
        ...
        if not has_weight_based and not has_fixed_iu and not has_ped_signal:
            continue
...
def main():
    ...
    name_placeholders = placeholders(VIFERON_NAMES)
    atc_placeholders = placeholders(VIFERON_ATC_CODES)
    ...
    groups = cur.execute(
        ...
        """,
        (*VIFERON_NAMES, *VIFERON_ATC_CODES),
    ).fetchall()
